...
export type Modality = 'inperson' | 'online' | 'hybrid'
export type DayKey = 'mon' | 'tue' | 'wed' | 'thu' | 'fri' | 'sat' | 'sun'
export type LangKey = 'es' | 'ca' | 'en' | 'fr'
...
const I18N: Record<UiLang, I18nEntry> = {
  ...
  ca: {
    ...
    labels: {
      ...
      days: { mon: 'Dilluns', tue: 'Dimarts', wed: 'Dimecres', thu: 'Dijous', fri: 'Divendres', sat: 'Dissabte', sun: 'Diumenge' },
      timeFrom: 'Des de',
      timeTo: 'Fins a',
      notes: 'Notes (opcional)',
      consent: 'He llegit i accepto la Política de Privacitat',
      nonEmergency: "Psicofinders no presta serveis d'emergència ni fa diagnòstic clínic. En cas d'urgència, truca al 112.",
  ...
  en: {
    ...
    labels: {
      ...
      nonEmergency: 'Psicofinders does not provide emergency services or clinical diagnosis. In case of emergency, call 112.',
  ...
  fr: {
    ...
    labels: {
      ...
      nonEmergency: "Psicofinders ne fournit pas de services d'urgence ni de diagnostic clinique. En cas d'urgence, appelez le 112.",
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-gray-50 text-gray-900">
      ...
      <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4">
        ...
        <div className="flex items-center gap-2">
          {(['es', 'ca', 'en', 'fr'] as UiLang[]).map((k) => (
            <button
              key={k}
              onClick={() => setUiLang(k)}
              className={`rounded-full border px-3 py-1 text-sm ${uiLang === k ? 'bg-gray-900 text-white' : 'bg-white'}`}
            >
              {I18N[k].langName}
            </button>
          ))}
      ...
      <section className="mx-auto grid w-full max-w-6xl grid-cols-1 items-center gap-10 px-4 py-10 md:grid-cols-2 md:py-16">
        <div>
          ...
          <a href="#form" className="mt-8 inline-block rounded-2xl bg-black px-6 py-3 text-white">
            {t.cta}
          </a>
        </div>
          ...
          <div className="rounded-3xl border bg-white p-6 shadow-md">
            <div className="grid grid-cols-2 gap-3">
              {SPECIALTIES.slice(0, 6).map((s) => (
                <div key={s} className="rounded-xl bg-gray-50 p-3 text-sm">
                  {s}
                </div>
              ))}
            </div>
      ...
      <section id="form" className="mx-auto w-full max-w-5xl rounded-3xl border bg-white px-4 py-8 shadow-sm md:px-8">
        ...
        <form ref={formRef} className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2" onSubmit={handleSubmit}>
          ...
          <Input
            ...
            onChange={(e) => handleChange('colegiado', e.currentTarget.value)}
          />
          ...
          <div className="col-span-1 md:col-span-2">
            <label className="text-sm font-medium">
              {t.labels.langs} <span className="text-red-600">*</span>
            </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input
              ...
              placeholder="40"
              value={form.priceMin}
              onChange={(e) => handleChange('priceMin', e.currentTarget.value)}
            />
            <Input
              ...
              placeholder="90"
              value={form.priceMax}
              onChange={(e) => handleChange('priceMax', e.currentTarget.value)}
            />
          </div>
          ...
          <div className="md:col-span-2 flex flex-col gap-3">
            <div className="flex items-start gap-2">
              <input
                ...
                onChange={(e) => handleChange('consent', e.currentTarget.checked)}
              />
          ...
          <div className="md:col-span-2">
            ...
            {status === 'ok' && (
              <div className="mt-3 rounded-xl border border-green-200 bg-green-50 p-3 text-green-900">{t.labels.success}</div>
            )}
            {status === 'err' && (
              <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-red-900">{t.labels.error}</div>
            )}
          </div>
        </form>
      </section>
